    ...
    def write(self):
        ...
        for letter in n:
            if letter == ' ':
                ...
                encryptedN += chr(ord(letter) + 2)

        for letter in e:
            if letter == ' ':
                encryptedE += ' '
            else:
                encryptedE += chr(ord(letter) + 2)

        for letter in p:
            if letter == ' ':
                encryptedP += ' '
            else:
                encryptedP += chr(ord(letter) + 2)
    ...
    def __init__(self, master, n, e, p, i):
        ...
        for letter in self.name:
            if letter == ' ':
                ...
                dencryptedN += chr(ord(letter) - 2)

        for letter in self.email:
            if letter == ' ':
                dencryptedE += ' '
            else:
                dencryptedE += chr(ord(letter) - 2)

        for letter in self.password:
            if letter == ' ':
                dencryptedP += ' '
            else:
                dencryptedP += chr(ord(letter) - 2)
...
def readfile():
    ...
    for line in f:
        List1 = line.split(',')
        e = entity_display(window, List1[0], List1[1], List1[2], count)
        objects.append(e)
        e.display()
        count += 1
    f.close()
